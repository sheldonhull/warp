...
use crate::ai::conversation_status_ui::{status_tint_color, STATUS_ELEMENT_PADDING};
...
pub fn render_item(props: ItemProps<'_>, app: &AppContext) -> Box<dyn Element> {
    ...
    // Unified agent icon-with-status circle (brand color + cloud lobe for
    // ambient runs) so the row matches the vertical tab / pane header. The
    // bazinga state-first treatment is applied inside `render_icon_with_status`
    // when `FeatureFlag::WarpBazingaSidebar` is on, so we don't need a separate
    // code path here.
    ...
    let bazinga_tint: Option<warpui::color::ColorU> =
        if FeatureFlag::WarpBazingaSidebar.is_enabled() {
            status_tint_color(&conversation.display.status.to_conversation_status(), theme)
        } else {
            None
        };
